...
Le socle de persistance PostgreSQL est engagé dans le code : schéma Prisma, repositories, orchestrateur de persistance, pagination keyset, lectures Prisma, publication et compensation atomiques, protection des routes internes et mécanique de livraison outbox. L’outbox sait réclamer les événements avec un bail optimiste, incrémenter les tentatives, orchestrer leur publication via un adaptateur, marquer un succès, replanifier un échec avec backoff exponentiel borné et jitter, et exécuter des cycles périodiques sans chevauchement via `LedgerOutboxWorker`. L’adaptateur broker définitif, le câblage du worker dans le processus d’infrastructure, les métriques/alertes, la dead-letter, la réconciliation et les validations PostgreSQL/concurrence restent à terminer.
