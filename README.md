...
Le socle de persistance PostgreSQL est engagé dans le code : schéma Prisma, repositories, orchestrateur de persistance, pagination keyset, lectures Prisma, publication et compensation atomiques, protection des routes internes et mécanique de livraison outbox. L’outbox sait réclamer les événements avec un bail optimiste, incrémenter les tentatives, orchestrer leur publication via un adaptateur, marquer un succès et replanifier un échec avec backoff exponentiel borné et jitter. Le processus périodique de transport, le broker définitif, la réconciliation, les scénarios de reprise et les validations PostgreSQL/concurrence restent à terminer.
...
Le dépôt plateforme contient le socle du monorepo, plusieurs contrats métier partagés et une tranche exécutable du ledger : persistance de référence, lectures de compte/solde/écritures, pagination keyset, publication et compensation atomiques, authentification interne transitoire, gestion du cycle de livraison outbox et orchestration avec backoff. Les applications exécutables, les interfaces mobiles et web, les adaptateurs partenaires, le worker périodique complet ainsi que les validations de production restent à construire progressivement.
